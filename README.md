...
これはbrother_qlというライブラリを職場用に改造したものです。

まず経典はこれです。
[brother公式　ラスターコマンドリファレンス]: https://download.brother.com/welcome/docp100278/cv_ql800_jpn_raster_100.pdf

### 改造点　202003
- --lqオプションで品質低・高速モードが働かない → --hqで品質高・低速モードということにしました
  * --lqフラグで速度重視モードになる、はずが全く変化が無いので、調べたところどうも引数の設定がおかしい。
  * --lqってオプションは、引数にlqって単語があるかどうかを見ないといけないはずなのに、hq（highqualityだろう）の有無でフラグ立てをしている。
  * なので、逆に、品質重視モードにしたいときに「--hq」フラグをつけるようにに変えました。デフォルトでlow quality = fast なモードで動きます。
- タイムアウトを120秒に変更しました。（もとは10秒）大量印刷向けです。
- 連続印刷時の挙動を改善し、速度がめちゃ早くなりました。
  * 元々は、毎回最後に切るかどうかは --nu-cutオプションで決めている。
  * 複数枚のときは、--no-cutなしだと毎回切る。--no-cutありだと、最後も切らない。
  * 本来は、複数ページを毎回カットor n枚ごとにカット or ノーカット、という「オートカット」フラグと、「カットアットエンド」という最後に切るかどうかのフラグの2種が存在。
  * なので、ルールを変更した。
    * 必ず最後にカットする。カットアットエンドは常にTrue
    * そレを実現するために、毎回最終ページとして命令を送っていたのを、本当に最終ページの場合のみ最終とするようにした（1A: 排紙を伴う印刷司令、それ以外は0C）
    * --no-cut → 毎ページカットしない。逆にこのオプションなしだと毎回カットする。

### 改造点　202006
ここで改善完了、としばらく本番運用を続けていると、なぜかたまにラベルロールが逆に吸い込まれて、鏡像を上書きするような挙動になることがあり、困っていた。枚数も正しく出ないので。
そこで分解したり、互換品であるラベルのマーキングの精度を疑ったり。いろいろ試したがわからず。
そんななか、P-touch editorでやってみると、すーーーっと印刷されていく。毎ページ止まって微調整してたのはなんだったんだろうか？

というわけで再び、ラスターコマンドリファレンスに戻って調査。
テストプリントを偽プリントサーバーに送信してバイナリデータをみて、brother_qlからも同じことを行ってデータを見比べてみると、いろいろ準拠していない。おもにconversion.pyを変更。

- ステータス情報リクエストを毎回行っていた。本来は印刷中に送るべきではないコマンドとのこと。そもそもネットワーク印刷の場合はステータス返ってこない→　初回だけにすることに。i == 0でリクエスト追加。
- 動的コマンドモード切替は本来毎ページ必要　→　上記の改造の中で抜け落ちていた。複数ページの場合のループの中に入れた
- 印刷情報指令の9個目のフラグ「先頭ページかどうか」が、常に0(先頭)に。原因はraster.pyでself.page_numberがカウントアップされないため。 →　init後は0,その後は1という決め打ちに。

これで試すとp-touch editor と同じような、スムーズな印刷が可能になった。
めでたしめでたし。
